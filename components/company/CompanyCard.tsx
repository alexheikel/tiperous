...
interface Props { company: Company; rank?: number; delay?: number; onClick?: () => void; useSlug?: boolean }

export default function CompanyCard({ company, rank, delay=0, onClick, useSlug=false }: Props) {
  ...
  function handleClick() {
    ...
    if (useSlug && (company as any).slug) router.push(`/c/${(company as any).slug}`)
    else router.push(`/company/${company.id}`)
  }
