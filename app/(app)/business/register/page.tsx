...
export default function BusinessRegisterPage() {
  ...
  if (profile?.is_business) return (
    <div style={{ textAlign:'center', padding:'60px 20px' }}>
      ...
      <h1 style={{ fontFamily:'Playfair Display,serif', fontWeight:900, fontSize:28, marginBottom:10 }}>
        Ya tenés cuenta Business
      </h1>
      <p style={{ color:'var(--muted2)', fontSize:15, marginBottom:24 }}>
        Tu cuenta está verificada y podés responder a tips de tu empresa.
      </p>
  ...
  return (
    <div style={{ maxWidth:460, margin:'0 auto' }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontFamily:'Playfair Display,serif', fontWeight:900, fontSize:26, marginBottom:8 }}>
          Cuenta Business
        </h1>
        <p style={{ color:'var(--muted2)', fontSize:14, lineHeight:1.7, marginBottom:20 }}>
          Respondé a los tips de tu empresa, construí tu reputación y mostrá tu lado de la historia.
        </p>
        <div style={{
      ...
      <div style={{ marginBottom:24 }}>
        {[
          ['💬', 'Respondé a cualquier tip sobre tu empresa'],
          ['🏆', 'Badge verificado en tu perfil'],
          ['📊', 'Acceso a estadísticas de tu empresa'],
          ['🚫', 'No podés borrar ni editar tips de usuarios'],
        ].map(([icon, text]) => (
      ...
      <div style={{ marginBottom:20 }}>
        <label style={{ color:'var(--muted2)', fontSize:11, fontWeight:700, letterSpacing:1, display:'block', marginBottom:8 }}>
          TU EMPRESA (OPCIONAL)
        </label>
        ...
        />
        {results.map(c => (
          <div key={c.id} onClick={()=>{ setSelected(c); setQuery(c.name) }} style={{
            ...
            marginBottom:6, fontSize:14, color:'var(--text)',
            transition:'all .15s',
          }}>
      ...
      <button onClick={handleCheckout} disabled={loading||!user} style={{
        ...redBtn, opacity: (loading||!user) ? 0.7 : 1,
      ...
      {!user && (
        <p style={{ textAlign:'center', marginTop:14, color:'var(--muted)', fontSize:13 }}>
          <a href="/login" style={{ color:'var(--red)', fontWeight:600 }}>Ingresá</a> o{' '}
          <a href="/register" style={{ color:'var(--red)', fontWeight:600 }}>creá una cuenta</a> para continuar
        </p>
      )}
    </div>
  )
...
const redBtn: React.CSSProperties = {
  ...
  boxShadow:'0 4px 24px rgba(232,52,28,0.35)',
}
