...
export default function RegisterPage() {
  ...
  return (
    <div style={{ minHeight:'100dvh', background:'#0c0c0e', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ w
cat > ~/Downloads/tiperous/app/\(auth\)/register/page.tsx << 'EOF'
'use client'
import { useRouter } from 'next/navigation'
import { createClient } from '@/lib/supabase/client'
import Link from 'next/link'
      ...
      <div style={{ width:'100%', maxWidth:380 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <button onClick={signInWithGoogle} style={{
            ...
            onMouseEnter={e=>{ e.currentTarget.style.transform='scale(1.02)'; e.currentTarget.style.boxShadow='0 6px 28px rgba(0,0,0,0.4)' }}
        ...
        <p style={{ textAlign:'center', marginTop:24, color:'rgba(255,255,255,0.3)', fontSize:13, lineHeight:1.6 }}>
          Al registrarte aceptás nuestros términos de uso.<br/>
          ¿Ya tenés cuenta? <Link href="/login" style={{ color:'#e8341c', fontWeight:600, textDecoration:'none' }}>Ingresar</Link>
        </p>
      </div>
    </div>
  )
